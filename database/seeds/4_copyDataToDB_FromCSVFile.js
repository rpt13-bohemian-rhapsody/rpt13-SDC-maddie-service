...
function insertSellersData() {
  try {
    db.runAnyStatement("COPY sellers(name) FROM '" + sellersFilePath + "' DELIMITER ',' CSV HEADER;", (res, err) => {
      if (err) {
        console.log('error ' + err);
      } else {
        console.log('Completed insertSellersData function : runAnyStatement copy : # of records : ' + (env.maxPrimary * env.maxSecondary).toLocaleString() + 
        ' records. time in seconds =  ' + (new Date().getTime() - startTime) / 1000);
      }
    });
  }
  catch (err) {
    console.log('ERROR insertSellersData : ' + JSON.stringify(err));
  }
}
...
function insertProductsData() {
  try {
    db.runAnyStatement("COPY products(name,description,product_price,seller_id) FROM '" + productsFilePath + "' DELIMITER ',' CSV HEADER;", (res, err) => {
      if (err) {
        console.log('error ' + err);
      } else {
        console.log('Completed insertProductsData function : runAnyStatement copy : # of records : ' + (env.maxPrimary * env.maxSecondary * 5).toLocaleString() + 
        ' records. time in seconds =  ' + (new Date().getTime() - startTime) / 1000);
      }
    });
  }
  catch (err) {
    console.log('ERROR insertProductsData : ' + JSON.stringify(err));
  }
}
